...
_data_dir_cache_context: tuple[str, str] | None = None


def find_data_dir() -> str:
    """Find the WeChat db_storage directory (most recent with valid key). Cached per process."""
    global _data_dir_cache, _data_dir_cache_context
    context = (
        config.DB_BACKEND,
        getattr(config, "DECRYPTED_DIR", "") if config.DB_BACKEND == "sqlite3" else config.WECHAT_DATA_GLOB,
    )
    if _data_dir_cache and _data_dir_cache_context == context and os.path.isdir(_data_dir_cache):
        return _data_dir_cache
    _data_dir_cache = None
    _data_dir_cache_context = context
    if config.DB_BACKEND == "sqlite3":
...
def _find_decrypted_data_dir() -> str:
    ...
    """
    selected_account = getattr(config, "WINDOWS_ACCOUNT", "").strip()
    if selected_account:
        selected_root = os.path.join(config.DECRYPTED_DIR, selected_account, "db_storage")
        selected_probe = os.path.join(selected_root, "message", "message_0.db")
        if os.path.isfile(selected_probe):
            return selected_root
        raise FileNotFoundError(
            f"指定账号没有已解密的 message_0.db: {selected_account}. "
            f"Expected: {selected_probe}"
        )
    hits = glob.glob(os.path.join(config.DECRYPTED_DIR, "**", "message", "message_0.db"), recursive=True)
    if not hits:
        raise FileNotFoundError(
