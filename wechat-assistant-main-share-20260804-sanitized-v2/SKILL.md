...
**待办提醒机制：** 新 todo 生成稳定 `fingerprint`（联系人 + 归一化摘要 + 截止时间）并按指纹去重。首次发现进入 `new`，推送后转为 `seen`；临期进入 `due_soon`，逾期进入 `overdue`，完成或取消进入 `done` / `dismissed`。明确截止时间超过 2 天仍未处理的 todo 自动标记 `done`；无截止时间、已确认/提醒过且创建超过 14 天的陈旧 todo 也自动标记 `done`。推送采用摘要式展示：只在新增、临期、逾期、完成变化或每日摘要时段提醒，不重复完整列出所有旧待办。
...
prompts/
  todo-scan.md               — 待办扫描 cron prompt（决策脑 v2：Layer A 状态感知 + 指纹去重 + 摘要式提醒）
...
```json
{
  "todos": {
    "items": [
      {"id": "todo_001", "fingerprint": "张三|确认合同|2026-04-20", "contact": "张三", "summary": "...", "status": "seen", "created": "2026-04-19", "resolved": null}
    ],
...
**去重策略**：
- **todos**: `existing_todos` 字段传给 prompt，LLM 按 `fingerprint` 和相似摘要去重。状态机：`new` → `seen` → `due_soon` / `overdue` → `done` / `dismissed` → 自动归档。无变化不推送，普通旧待办只在每日摘要中最多展示 3 条
...
> ```
> todo-scan 无变化时不发心跳；其他 cron 是否发送状态消息由各自 prompt 决定。

#### Cron 1: 待办扫描（每天 10:00 / 15:00 / 20:00）
- 模板：`prompts/todo-scan.md`
- Schedule: `0 10,15,20 * * *`
- 状态：按 `fingerprint` 对比已有 todos，只在新增、临期、逾期、完成变化或每日摘要时段推送
...
密钥首次提取后保存到 `all_keys.json`，后续恢复复用该文件。
如果本机微信重新登录或 WAL checkpoint 后需要恢复，运行 `refresh_decrypt.py --full` 强制全量解密。
Agent 如果发现 `refresh_decrypt.py` 报错（HMAC 验证失败），应先判断本机微信是否登录完成、DB 是否仍在同步；不要自动重新提取密钥。

如果用户在其他设备登录导致本机微信下线，恢复顺序是：先在本机重新登录桌面微信，等待最近聊天同步完成，再运行 `scripts/recover_wechat_login.sh`。Hermes cron 使用 `scripts/wechat_health_gate.py` 做前置检查；它会复用已有密钥尝试全量刷新并做 168 小时恢复同步。若本机微信未登录完成或 DB 暂不可读，gate 应输出 `wakeAgent:false` 并跳过本轮，避免继续调用 LLM 造成重复失败。
...
collector.db 存数字格式（`12345678@chatroom`），但 config 可能填了 hash 格式（`e8ece7ec82b022283b74d948e1ba7870@chatroom`）。
...
# 3. 查看特定私聊的 sync_state（如林杰 wxid_example_contact）
sqlite3 ~/wechat-assistant/collector.db \
  "SELECT last_local_id, datetime(last_sync_at, 'unixepoch', 'localtime') FROM sync_state WHERE chatroom_id='wxid_example_contact'"

# 4. 确认解密DB里是否有该私聊的新消息（collector用md5(chatroom_id)映射表名）
python3 -c "
import hashlib, sqlite3
wxid = 'wxid_example_contact'
...
python3 collector.py --config ~/wechat-assistant/config.yaml --sync --chatroom wxid_example_contact
```
...
**原则**：todo-scan 不再每次展示全部旧 todo；只推送新增、临期、逾期、完成变化，以及每日摘要时段的精简总览。
- 无变化且不在摘要时段时不推送
- 新增只提醒一次，后续重复出现只更新 `mention_count` 和 `last_seen_at`
- 普通旧待办只在摘要中最多展示 3 条
...
cur = conn.cursor()
for uid in ['12345678@chatroom']:  # 替换为上一步查到的 username
...
todo-scan 必须保留足够上下文，但不要重复刷屏：

- **有变化时**：标题写 "微信待办摘要"，分类显示新增/临期/逾期/完成/仍在进行
- **无变化时**：不在摘要时段则不推送；在摘要时段只发精简总览
- **新增标记**：`🔔` 表示新待办或待确认更新
- **临期/逾期标记**：`⏰` / `⚠️` 表示需要再次关注
- **天数标注**：超3天的 todo 只在摘要中最多展示 3 条
