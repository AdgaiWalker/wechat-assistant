...
class StateManager:
    ...
    def _parse_datetime(self, value, end_of_day=False):
        """Parse common todo datetime/date/timestamp values into TZ+8 datetimes."""
        if value is None or value == '':
            return None
        if isinstance(value, (int, float)):
            try:
                return datetime.fromtimestamp(int(value), tz=_TZ8)
        ...
        if text.isdigit():
            try:
                return datetime.fromtimestamp(int(text), tz=_TZ8)
            except (ValueError, TypeError, OSError):
                return None
    ...
    def _todo_deadline(self, item):
        for key in ('deadline', 'due_at', 'due_date', 'event_date'):
            parsed = self._parse_datetime(item.get(key), end_of_day=True)
            if parsed:
                return parsed
        return None

    def _todo_created_at(self, item):
        for key in ('created_ts', 'created', 'created_date'):
            parsed = self._parse_datetime(item.get(key))
            if parsed:
                return parsed
        return None

    def auto_complete_expired_todos(self, grace_days=2, stale_days=14):
        """Mark old unresolved todos as done so they stop recurring forever.

        Rules:
        - deadline older than ``grace_days`` gets auto-completed.
        - no-deadline todos that were already acknowledged/notified and are
          older than ``stale_days`` get auto-completed.
        """
        state = self._read()
        now = datetime.now(tz=_TZ8)
        deadline_cutoff = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=grace_days)
        stale_cutoff = now - timedelta(days=stale_days)
        stats = {'total': 0, 'deadline_expired': 0, 'stale_no_deadline': 0}

        for item in state.get('todos', {}).get('items', []):
            if item.get('status') not in _OPEN_TODO_STATUSES:
                continue

            reason = None
            deadline = self._todo_deadline(item)
            if deadline and deadline < deadline_cutoff:
                reason = 'deadline_expired'
            elif not deadline:
                created = self._todo_created_at(item)
                was_seen = (
                    item.get('acknowledged')
                    or item.get('reminder_state') == 'notified'
                    or bool(item.get('last_notified_at'))
                )
                if was_seen and created and created < stale_cutoff:
                    reason = 'stale_no_deadline'
            ...
            item['auto_completed_reason'] = reason
            item['auto_completed_at'] = now.isoformat()
            stats['total'] += 1
            stats[reason] += 1
    ...
    def ack_all_todos(self):
        ...
        count = 0
        for item in state['todos']['items']:
            if item.get('status') in _OPEN_TODO_STATUSES and not item.get('acknowledged'):
                item['acknowledged'] = True
                item['ack_time'] = datetime.now(tz=_TZ8).isoformat()
                count += 1
        if count > 0:
    ...
    def infer_user_status(self):
        ...
        urgent_open = [t for t in todos if t.get('status') in _OPEN_TODO_STATUSES and t.get('urgent')]
        if urgent_open:
