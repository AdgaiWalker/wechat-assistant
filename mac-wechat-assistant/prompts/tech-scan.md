...
`ok_after_full_refresh` 时才继续。**不要再次运行 `refresh_decrypt.py`**，避免在微信
写库窗口中重复读取数据库并产生瞬时 HMAC 误报。
...
> 健康检查未通过时 scheduler 会跳过本轮。HMAC 失败表示数据库当前不可验证，
> 不应直接断言“密钥过期”或要求重新提取密钥。
...
```bash
python3 extract_tech.py --config ~/wechat-assistant/config.yaml
```
...
```bash
python3 ~/.hermes/skills/social-media/wechat-assistant/scripts/db_writer.py --db ~/wechat-assistant/assistant.db --table tech_highlights --data '[{items: category, keyword, count, groups, highlights(JSON array)}]'
python3 ~/.hermes/skills/social-media/wechat-assistant/scripts/db_writer.py --db ~/wechat-assistant/assistant.db --scan-log "tech:ok:X分类Y条有效讨论"
```
