...
# collector.py 只从 Msg_* 表同步文本消息。资源库只保存附件/媒体元数据，
# 旧的已解密副本仍可保留使用，但它暂时不可解密时不应阻断文本扫描任务。
OPTIONAL_DATABASES = frozenset({
...
def verify_page1_hmac(db_path, enc_key):
    ...
    返回 True=密钥可验证, False=文件暂不可验证或已损坏。
    本项目复用已保存密钥；验证失败时通常先等待微信登录/同步完成后重试。
    """
...
def main():
    ...
    # 失败项必须保留旧 mtime（或保持缺失），确保下一次运行仍会重试。
    # 不能提前写入当前 mtime，否则一次 HMAC 失败会在下次被误判为“无需刷新”。
    ...
    for rel, src_path in db_files:
        ...
        if need_full or db_changed:
            # 全量解密前先验证 HMAC（确认 DB 已可用）
            try:
                if not verify_page1_hmac(src_path, enc_key):
                    if rel in OPTIONAL_DATABASES:
                        print(f'[WARN] 可选数据库 HMAC 验证失败 {rel} — 保留已有解密副本并稍后重试',
                              file=sys.stderr)
                        optional_hmac_fail_count += 1
                    else:
                        print(f'[ERROR] HMAC 验证失败 {rel} — 数据库暂不可验证（微信登录/同步未完成？）',
                              file=sys.stderr)
                        hmac_fail_count += 1
                    continue
            ...
            # WAL patch 前也要验证 HMAC（避免微信登录/同步中的临时 DB 状态污染输出）
            try:
                if not verify_page1_hmac(src_path, enc_key):
                    if rel in OPTIONAL_DATABASES:
                        print(f'[WARN] 可选数据库 HMAC 验证失败 {rel} — 保留已有解密副本并稍后重试',
        ...
        # 只有成功解密、成功 patch 或确认无需处理时，才提交本次观察到的 mtime。
    ...
    if hmac_fail_count > 0:
        print(
            f'[WARN] {hmac_fail_count} 个 DB HMAC 验证失败！'
            '请确认本机微信已登录且聊天同步完成，然后重试',
            file=sys.stderr
        )
        sys.exit(2)  # 特殊退出码，Agent 可据此提醒用户

    if optional_hmac_fail_count > 0:
        print(
            f'[WARN] {optional_hmac_fail_count} 个可选 DB 暂不可解密；'
            '文本消息同步可继续，下次刷新会自动重试',
            file=sys.stderr
        )
