...
def main():
    ...
    # 先做硬性过期清理，避免老待办继续喂给 LLM 反复提醒。
    auto_completed = sm.auto_complete_expired_todos(grace_days=2, stale_days=14)

    # 从 scan_state 读取已有 open todos
    todo_state = sm.get_todos()
    open_statuses = {'open', 'new', 'seen', 'due_soon', 'overdue'}
    existing_todos = [t for t in todo_state.get('items', []) if t.get('status') in open_statuses]
    ...
    output = {
        ...
        'auto_completed_expired': auto_completed,
