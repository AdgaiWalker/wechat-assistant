...
def _message_dedupe_key(sender, content, urls=None):
    if not sender:
        return None

    urls = tuple(sorted(set(urls if urls is not None else _extract_urls(content or ''))))
    if urls:
...
def main():
    ...
    for chatroom_id, sender, content, msg_time, msg_type, group_name in rows:
        ...
        dedupe_key = _message_dedupe_key(sender, content, urls)
        if dedupe_key:
